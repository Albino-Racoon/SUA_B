...
# -*- coding: utf-8 -*-
"""
Izboljšana skripta za čiščenje komentarjev v CSV datoteki.
...
def clean_comments(input_file, output_file=None):
    """
    Temeljito očisti komentarje v CSV datoteki z odstranitvijo vsega nepotrebnega besedila.
    ...
    # Vzorci za prepoznavanje nepotrebnega besedila
    patterns_to_remove = [
        # HTML in spletno besedilo
        r'Fakulteta za elektrotehniko, računalništvo in informatiko / UNI',
        r'Število ocen:\d+',
        r'Predmeti:.*?dodaj predmet',
        r'Komentiraj predmete! Klikni na ime predmeta zgoraj!',
        r'Spremeni podatke profesorja',
        r'Dodaj komentar',
        r'Sponzorji',
        r'O tem profesorju še nihče ni napisal mnenja\.',
        r'<.*?>',  # HTML tagi
        r'https?://[^\s]+',  # URL-ji
        
        # Vzorci za "moč komentarja"
        r'moč komentarja:-\d+\+\d+',
        r'moč komentarja:\d+',
        
        # Vzorci za "Napisal:"
        r'Napisal:[^\s]+',
        
        # ASCII art in nepotrebni simboli
        r'[⠁-⣿]+',  # Braille vzorci
        r'[▄▀█░]+',  # ASCII art vzorci
        
        # Neznani vzorci
        r'[^\w\s\.,!?;:()"\'\-]',  # Ohrani samo črke, številke, presledke in osnovno ločila
    ]
                ...
                for row_num, row in enumerate(reader, start=2):  # start=2 ker je prva vrstica glava
                    ...
                    try:
                        ...
                        if 'komentar' in row and row['komentar']:
                            ...
                            # Odstrani vse vzorce nepotrebnega besedila
                            for pattern in patterns_to_remove:
                                comment = re.sub(pattern, '', comment, flags=re.IGNORECASE | re.DOTALL)
                            
                            # Očisti presledke in prazne vrstice
                            comment = re.sub(r'\s+', ' ', comment)  # Več presledkov v en
                            comment = comment.strip()  # Odstrani presledke na začetku in koncu
                            
                            # Preveri, ali je komentar po čiščenju smiseln
                            if len(comment) > 5 and not comment.isspace():
...
def main():
    ...
    print("Izboljšano čiščenje komentarjev v CSV datoteki...")
